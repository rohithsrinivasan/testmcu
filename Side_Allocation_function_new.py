...
def balance_grid_space(df):
    ...
    if not left_groups or not right_groups:
        ...
        # Reassign half the groups to the opposite side
...
def assigning_side_for_less_than_80_pin_count(df):
    ...
    # Reset 'Side' if already present
    if 'Side' in df.columns and df['Side'].notna().any():
        print("Info: 'Side' column already has values. Resetting to NaN before reassigning.")
        df['Side'] = pd.NA
